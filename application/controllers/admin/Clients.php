...
class Clients extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('admin/login_model', 'cms/clients_model'));
	...
	public function index($offset=0)
	{
		$limit = 15;
		$filters = array();
		$sql_properties = $this->clients_model->sql_properties($limit, $offset);
		...
		$this->data['pagination'] = $this->batch_model->pagination($this->table, $filters, "#clients/index/", $limit, 4, $total_results);
		$this->batch_model->render_page("admin/addons/clients/clients", $this->data);
	}
	...
	public function manage($mode='add', $id=0)
	{
		...
		if ($this->form_validation->run() == TRUE) {
			$parsed_data = $this->clients_model->parsed_data($mode, $id);
		...
		$sql_properties = $this->clients_model->sql_properties(1);
		$sql_properties_for_lang = $this->clients_model->sql_properties(100);
		if($mode == 'delete') {
		...
		$this->batch_model->render_page("admin/addons/clients/manage-client", $this->data);
	}

	public function _delete_image($id=0)
	{
		$return_data = $this->batch_model->get_rows(array('table'=>$this->table, 'limit'=>1), array('id'=>$id));
		if(isset($return_data['image'])) { @unlink("./uploads/clients/{$return_data['image']}"); }
		return;
	}
