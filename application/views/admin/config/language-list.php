...
<script type="text/javascript">
	jQuery(document).ready(function() {
		...
		function processJson(data) {
		    ...
		    if(obj.mcode == 200) {
		    	$("#smart-form-zone").attr("action", "<?= site_url("controlpanel/languages/index"); ?>/" + obj.data_mode + "/" + obj.zone_id);
		...
		jQuery("#jqgrid").jqGrid({
			/*data : jqgrid_data,*/
			url:'<?= site_url("controlpanel/languages/cdata"); ?>',  
			datatype : "json",
			mtype : "get", 
			height : 'auto',
 			loadComplete: function () {                                         
 			},
			colNames : ['ID', 'Language', 'Code', 'Direction', 'Status'],
			colModel : [{
				name : 'id',
				index : 'id',
				sortable : true
			...
			editurl : "<?= site_url('controlpanel/languages/index/edit'); ?>",
			caption : "Languages",
			multiselect : true,
			autowidth : true,
		});
