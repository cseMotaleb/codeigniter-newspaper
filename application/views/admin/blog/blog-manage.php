	  		...
	  		<div class="pull-right">
				...
				<select name="enabled" id="enabled" style="color: #000;">
					<option value="1" <?php if(isset($row_data['enabled']) && $row_data['enabled'] == '1') echo 'selected'; ?>>Enabled</option>
                                        <option value="2" <?php if(isset($row_data['enabled']) && $row_data['enabled'] == '2') echo 'selected'; ?>>Pending</option>
					<option value="0" <?php if(isset($row_data['enabled']) && $row_data['enabled'] == '0') echo 'selected'; ?>>Disabled</option>
				</select>
                            <?php } else { ?>
                            <input type="hidden" name="enabled" value="<?php if(isset($row_data['enabled']) && $row_data['enabled']) echo $row_data['enabled']; else echo '2'; ?>" />
                            <?php } ?>
	  		</div>
	  	...
	  	<div class="panel-body">
			    		...
			    		<div class="row">
			    			<div class="col-md-6">
								...
								<select name="category_id[]" id="category_id<?= $i; ?>" class="select2">
					    	    	...
					    	    	<?php
					    	    		$category_rows = get_rows(array('table'=>"blog_groups", "limit"=>100, "oder_by"=>"position", "order_type"=>"asc"), array("main_menu"=>1, 'parent_id'=>0));
					    	    		foreach ($category_rows as $key => $category) {
					    	    			$selected = (isset($categories[$i]['category_id']) && ($categories[$i]['category_id'] == $category['id'])) ? 'selected="selected"' : "";
											echo "<option value=\"{$category['id']}\" {$selected}>{$category['category']}</option>";
										}
					    	    		$category_rows = get_rows(array('table'=>"blog_groups", "limit"=>100, "oder_by"=>"category", "order_type"=>"asc"), array("main_menu !="=>1, 'parent_id'=>0));
					    	    		foreach ($category_rows as $key => $category) {
					    	    			$selected = (isset($categories[$i]['category_id']) && ($categories[$i]['category_id'] == $category['id'])) ? 'selected="selected"' : "";
											echo "<option value=\"{$category['id']}\" {$selected}>{$category['category']}</option>";
										}
    		...
    		<div class="row">
				...
				<div class="col-md-2">
    				<div class="formSep">
						<label>সর্বশেষ</label>
						<select name="recent" id="recent" class="select2">
							<option value="0" <?php if(isset($row_data['recent']) && $row_data['recent'] == 0) echo 'selected'; ?>>No</option>
							<option value="1" <?php if(isset($row_data['recent']) && $row_data['recent'] == 1) echo 'selected'; ?>>Yes</option>
						</select>
					</div>
				</div>
	    	</div>
			...
			<div class="row">
				<div class="col-md-4">
					<div class="formSep">
						...
						<select name="publisher" id="publisher" class="select2">
							<option value="0" <?php if(isset($row_data['publisher']) && ($row_data['publisher'] == 0)) echo 'selected="selected"'; ?>>Hide Publisher</option>
							<option value="1" <?php if(isset($row_data['publisher']) && ($row_data['publisher'] == 1)) echo 'selected="selected"'; ?>>Show Publisher</option>
						</select>
					</div>
				</div>
				<div class="col-md-4">
					<div class="formSep">
						<label>Editor/প্রতিনিধি</label>
						<input type="text" class="form-control" placeholder="Enter Editor/প্রতিনিধি" name="publisher_title" id="publisher_title" value="<?php if(isset($row_data['publisher_title'])) echo $row_data['publisher_title']; ?>" />
					</div>
				</div>
				<div class="col-md-4">
					<div class="formSep">
						<label>প্রতিনিধি </label>
						<input type="text" class="form-control" placeholder="Enter agent details" name="agent" id="agent" value="<?php if(isset($row_data['agent'])) echo $row_data['agent']; ?>" />
					</div>
				</div>
			</div>
			...
			<?php } else { ?>
			<div class="formSep">
				<small class="text-danger">ছবির সর্বনিম্ন সাইজ 200  বাই 200  পিক্সেল</small>
				
				<br />
				
				<label>Add Images (Default Image size : 600px * 450px)</label>
						    ...
						    <div class="panel panel-default">
					    		...
					    		<table class="table table-bordered table-striped">
									...
									foreach ($row_data['images'] as $key => $image_row) {
										...
										$label = ($image_row['enabled'] == 1) ? 'label-success' : (($image_row['enabled'] == 2) ? "label-warning" : 'label-danger');
										$status = ($image_row['enabled'] == 1) ? 'Enabled' : (($image_row['enabled'] == 2) ? "Not Show Details" : 'Disabled');
									?>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		jQuery('.rimgstatusopt').editable({
		    url      : "<?= site_url("admin/news/ajax_update_image_status"); ?>",
		    success  : function(response, newValue) {             
		       processJson(response);
		    },
		    source: [{value: '1', text: 'Enabled'}, {value: '2', text: 'Not Show Details'}, {value: '0', text: 'Disabled'}],  
		});
