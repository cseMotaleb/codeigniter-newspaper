<div class="row">
	<div class="col-lg-6">
		<h1 class="page-title txt-color-blueDark">
			<i class="fa-fw fa fa-list"></i> News Position (<?= $category_data['category']; ?>)
		</h1>
	</div>
	<div class="col-lg-6">
...
<form id="validate-form" method="post" action="<?= site_url("admin/news/position/{$category_id}"); ?>" role="form" data-toggle="validator">
	<div class="panel panel-primary">
		...
		<div class="panel-body">
			...
			<table id="ItemTable" class="table table-bordered">
	        	...
	        	if($total_rows > 0) {
	        		...
	        		foreach ($news as $row) {
	        			...
	        			if(!isset($highlight[$row['id']]['id'])) $highlight[$row['id']] = get_rows(array("table"=>"highlight_blog", "limit"=>1), array("category_id"=>$category_data['id'], "blog_id"=>$row['id']));
			...
			<?php /*
			<table style="width: 100%;" class="appendo" data-rows="<?= (isset($highlight_blogs) && count($highlight_blogs) > 0) ? (count($highlight_blogs) + 1) : 1; ?>">
				<?php
				if(isset($highlight_blogs) && count($highlight_blogs) > 0) {
					foreach ($highlight_blogs as $key => $h_blog) {
				?>
			  	<tr class="inv_row">
			    	<td>
						<div class="row">
							<div class="col-md-9">
								<div class="form-group">
									<select name="news[]" class="select2">
										<?php
										foreach ($news as $key => $row) {
											$selected = ($h_blog['blog_id'] == $row['id']) ? 'selected="selected"' : "";
										?>
							...
							<div class="col-md-3">
								<div class="form-group">
									<input class="form-control" type="text" name="position[]" placeholder="Position" value="<?= $h_blog['position']; ?>" />
								</div>
							</div>
						</div>
			  	...
			  	<tr class="inv_row">
			    	<td>
						<div class="row">
							<div class="col-md-9">
								<div class="form-group">
									<select name="news[]" id="item0" class="select2">
		...
		<div class="panel-footer">
			<div class="text-center">
				...
				<button class="btn btn-lg btn-primary" type="submit">Save News Position</button>
			</div>
		</div>
	</div>
</form>
