<div class="row">
	<div class="col-lg-6">
		<h1 class="page-title txt-color-blueDark">
			<i class="fa-fw fa fa-list"></i> Home 9 News Position
		</h1>
	</div>
	<div class="col-lg-6">
...
$news_rows = array();
if($total_rows > 0) {
	$position_wise_news = array();
	$without_position_news = array();

	$highlight = array();
	foreach ($news as $row) {
		if(!isset($highlight[$row['id']]['id'])) $highlight[$row['id']] = get_rows(array("table"=>"highlight_blog", "limit"=>1), array("page"=>"Home", "blog_id"=>$row['id']));
		$row['blog_url'] = site_url("article/{$row['id']}");

		if(isset($highlight[$row['id']]['position'])) $position_wise_news[] = $row;
		else $without_position_news[] = $row;
		
		$news_rows = @array_merge($position_wise_news, $without_position_news);
	}
}*/
?>
